...
type
  TfraCopyFiles = class(TFrame)
    ...
    procedure SimpleFileCopySkippedFile(Sender: TObject;
      aFileNameOrFolder: string);
  protected
    FdlgBusy : TdlgBusy;
    FIsFreeing : boolean;
...
if assigned(dlgBusy) then FreeAndNil(dlgBusy);
...
procedure TfraCopyFiles.SimpleFileCopySkippedFile(Sender: TObject;
  aFileNameOrFolder: string);
begin
LogEvent('Skipped File: ' + aFileNameOrFolder);
end;
...
if not Assigned(dlgBusy) then
   Begin
     dlgBusy := TdlgBusy.Create(self);
     dlgBusy.Show;
     dlgBusy.fraBusy.jvGIFAnimation.Animate := True;
   End
else
   begin
     if self.cbShowProgress.Checked then Application.ProcessMessages;
